...
namespace TaskLayer
{
    ...
    public abstract class MetaMorpheusTask
    {
            ...
            out int emptyEntriesCount, CommonParameters commonParameters)
        {
            ...
            DecoyType decoyDontGenerateForShuffleYet = decoyType == DecoyType.Shuffle ? DecoyType.None : decoyType;
            if (theExtension.Equals(".fasta") || theExtension.Equals(".fa"))
            {
                um = null;
                proteinList = ProteinDbLoader.LoadProteinFasta(fileName, generateTargets, decoyDontGenerateForShuffleYet, isContaminant, ProteinDbLoader.UniprotAccessionRegex, 
                    ProteinDbLoader.UniprotFullNameRegex, ProteinDbLoader.UniprotFullNameRegex, ProteinDbLoader.UniprotGeneNameRegex, ProteinDbLoader.UniprotOrganismRegex, out dbErrors, commonParameters.MaxThreadsToUsePerFile);
            }
            else
            {
                List<string> modTypesToExclude = GlobalVariables.AllModTypesKnown.Where(b => !localizeableModificationTypes.Contains(b)).ToList();
                proteinList = ProteinDbLoader.LoadProteinXML(fileName, generateTargets, decoyDontGenerateForShuffleYet, GlobalVariables.AllModsKnown, isContaminant, modTypesToExclude, out um, commonParameters.MaxThreadsToUsePerFile);
            }
